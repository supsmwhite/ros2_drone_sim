...
常规 anti-windup 使用 back-calculation：`integral_dot = Ki·position_error + Kaw·(a_saturated-a_raw)`，水平输出饱和时冻结误差积分但允许回算卸载；积分向量另有独立限幅。节点在启动、无有效目标、地面待机、大 pose goal 跳变、无效状态和控制失效时 reset；Odom 短时超时、捕获半径外、水平饱和及快速轨迹阶段 freeze。连续 trajectory 采样不会被当成目标跳变。节点还从连续 Odom 估计水平加速度变化，在已有积分储量的等效偏置突然撤销时开启 `8 s` 去积分窗口，以 Kaw 渐进卸载，而不订阅或依赖仿真外力 Topic。诊断消息分别发布 P/D/I/前馈、raw/limited 加速度以及 enabled/frozen/reset/anti-windup 状态。

同口径 PD→最终方案对比：`0.30 N × 2 s` 最大偏移 `0.34862→0.33352 m`、恢复 `5.725→5.500 s`；`0.80 N × 2 s` 最大偏移 `0.95020→0.93680 m`、恢复 `7.543→7.536 s`，高度/姿态/Mixer 均无饱和。`0.30 N × 15 s` 最后 3 秒平均水平误差由 `0.74934 m` 降至 `0.08199 m`，平均速度 `0.01058 m/s`，平均积分补偿约 `-0.3138 m/s²`。`0.30 N × 10 s` 撤力后反向超调 `0.15857 m`，最终误差/速度 `0.00309 m / 0.00373 m/s`。

默认无扰动三目标仍只作为最终回归：完成 `139.204 s`，最大跟踪误差 `0.026826 m`，最小基础净空 `0.094343 m`，最终误差/速度 `0.004221 m / 0.005364 m/s`，控制饱和为零。动力学质量改为 `1.10/1.20 kg` 而控制器仍按 `1.00 kg` 时，水平保持不漂移，但高度分别稳定在 `1.1731/0.8462 m`；水平积分没有解决高度质量失配。高度环和姿态环仍是 PD，不能把整个飞控称为完整 PID。外力仍是集中等效力，不是障碍地图中的完整风场。

最终全量回归为 `276 tests, 0 errors, 0 failures, 0 skipped`，`drone_bringup` 的 14 个 Launch 测试全部通过。交互导航完成 `49.960 s`，最大跟踪误差 `0.019958 m`、最小净空 `0.241833 m`、最终误差/速度 `0.003702/0.004641`，饱和为零。预检失败保持 setpoint `0`、404 条 RPM 命令最大值 `0`、高度和水平位移 `0`。
