...
- `multi_goal_static_avoidance_sim.launch.py` 的唯一多目标规划执行链路，以及独立 Domain 113 四目标三维真实闭环安全回归；
- MotorRPM 命令超时保护；
...
- 最终验收环境为 `14 × 9 × 5.5 m`，包含三个交替连接边界的全高墙和三个内部独立障碍物；O5/O6 形成原始 `1.9 m`、膨胀后 `1.2 m` 的挑战通道，不能从同一外围一次绕过全部障碍物；
- 默认场景的三维 26 邻域 A* 使用 `0.25 m` 基础安全半径和 `0.10 m` 规划裕量，按 `0.35 m` 有效半径检查节点和边；从 `(0,0,1.5)` 到 `(12.1,0.85,1.5)` 的确定性结果为 `55` 个路径点、长度 `16.069147 m`、约扩展 `7680` 个节点，最大高度 `1.6 m`；
- 默认原始路径可确定性简化为 `8` 个点、长度 `15.216305 m`；连续轨迹选择 `velocity_scale=1.00`，总时长 `43.877966 s`，最大速度 `0.552505 m/s`、最大加速度 `0.321714 m/s²`，采样点及其相邻连线均通过 `0.35 m` 有效规划碰撞模型；
- 用户已在 RViz2 中人工确认黄色 A* 原始栅格路径、粉色视线简化折线和蓝色连续参考轨迹能够同时显示；蓝色连续参考轨迹整体平滑，符合当前路径轨迹化预期。视觉观察不用于精确测量安全距离，几何安全仍以自动碰撞验证为依据；
- Domain 99 静态避障执行回归中，无人机先在 `(0,0,1.5)` 稳定，再按 `0→1→2→3→4→5→6` 执行规划轨迹并到达 `(12.1,0.85,1.5)`；任务于 `48.194 s` 完成，最大跟踪误差 `0.029918 m`、对基础膨胀障碍物的最小净空 `0.183089 m`、最终误差 `0.005209 m`、最终速度 `0.001550 m/s`，完成后继续保持至少 `3.0 s`，控制器日志未出现饱和；
...
- Domain 113 有序多目标回归从实际地面 Odom 原地起飞，依次到达 P1 `(3.6,3.1,1.5)`、P2 `(7.85,3.1,2.5)`、P3 `(9.85,0.6,3.25)`、P4 `(12.1,0.85,1.5)`；四段分别展示绕 O1、在 O2/O3 间连续换向、下穿 O4，以及穿过 O5/O6 挑战通道并下降。任务于 `83.957 s` 完成，各段最大跟踪误差为 `0.013086/0.015679/0.015579/0.011159 m`，最小净空 `0.209253 m`，最终误差 `0.001978 m`、最终速度 `0.000907 m/s`；全程无碰撞、非有限值或控制饱和；
- 当前四目标任务已由 Domain 113 闭环回归确认严格按序执行；RViz2 保留多目标路径、实际轨迹和环境显示，当前六障碍地图的定量验收以自动测试和评测数据为准；
- RViz2 的 Orbit 焦点与 `16 m` 观察距离适用于未扩大的工作空间，可同时显示完整工作空间、六个原始障碍物、透明安全膨胀区、无人机及四类路径；
...
环境配置位于 `src/drone_bringup/config/environment.yaml`。默认 workspace 为 `[-1,13] × [-2.5,6.5] × [-0.5,5] m`，三个边界连接高墙与三个内部独立高障碍物形成水平蛇形路线和局部挑战通道；默认 A* 契约为 `(0,0,1.5) → (12.1,0.85,1.5)`，分辨率保持 `0.25 m`。`/drone/environment/markers` 显示工作空间、原始障碍物和透明安全膨胀区；`/drone/environment/in_collision` 只在收到有限且未超时的 Odom 时发布。无图形界面时可增加 `use_rviz:=false`。
...
默认任务配置 `multi_goal_mission.yaml` 依次包含 `(3.6,3.1,1.5)`、`(7.85,3.1,2.5)`、`(9.85,0.6,3.25)` 和 `(12.1,0.85,1.5)` 四个零 yaw 目标；当前节点显式要求 yaw 为零，因此未填写无效航向变化。可用 `mission_config:=<绝对路径>` 载入独立任务 YAML。节点启动时一次性读取目标列表，以首次有效 Odom 的 x/y 为起飞锚点，先发布 `z=1.5 m` 的静止 setpoint；位置与速度连续稳定 `1.0 s` 后，才从当时实际 Odom 规划 P1。每个目标同样采用“到达、停稳、切换”语义，Odom 无效或超时会暂停当前轨迹时钟，最终持续保持最后目标。任务状态可通过 `/drone/multi_goal/current_goal_index`、`current_segment`、`complete`、`success` 和 `visited_goals` 五个 Topic 观察。
