...
核心算法与 ROS2 通信层分离，动力学、控制器、Motor Mixer、多目标点任务管理、分段五次轨迹，以及静态环境几何与碰撞检查均可独立测试。项目后续目标包括三维栅格路径规划和静态避障。

## 当前阶段

动力学、高度/yaw、单目标三维位置闭环、第一版多目标点顺序飞行、连续轨迹生成与跟踪，以及静态三维 AABB 环境与统一碰撞查询均已完成。`environment_sim.launch.py` 可显示工作空间、原始障碍物和按无人机安全半径膨胀后的区域，并实时报告当前位置的几何碰撞状态；该状态目前不介入动力学或电机控制。
...
- `basic_sim.launch.py` 一键启动动力学、控制器、机器人模型发布和 RViz2；`mission_sim.launch.py` 启动离散顺序任务，`trajectory_sim.launch.py` 启动连续轨迹任务，`environment_sim.launch.py` 启动基础仿真和静态环境监测。
...
- 当前工作区最近一次完整测试结果为 `165 tests, 0 errors, 0 failures, 0 skipped`。
...
环境配置位于 `src/drone_bringup/config/environment.yaml`。默认 workspace 为 `[-1,9] × [-2.5,7.5] × [-0.5,5] m`，两个错开的墙状障碍物为下一阶段 `(0,0,1.5) → (8,5,1.5)`、建议 `0.25 m` 栅格分辨率的 A* 场景预留周围和上方通道；这些起终点尚未成为 ROS2 参数。`/drone/environment/markers` 显示工作空间、原始障碍物和透明的安全膨胀区；`/drone/environment/in_collision` 只在收到有限且未超时的 Odom 时发布。无图形界面时可增加 `use_rviz:=false`。
...
- 当前静态环境只提供几何查询、可视化和状态报告，不会阻止电机、改变动力学或产生物理撞击响应；
- 尚未实现三维栅格、A*、路径平滑、自动避障、在线重规划或动态障碍物；
