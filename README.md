...
- 最终验收环境为 `14 × 9 × 5.5 m`，包含五个全高墙段组成的三道屏障：第一道迫使上绕，第二道形成 `1.5 m` 有效中部通道，第三道形成 `1.8 m` 有效上部通道；上下边界交替封闭，不能从同一外围一次绕过全部障碍物；
- 默认场景的三维 26 邻域 A* 使用 `0.25 m` 基础安全半径和 `0.10 m` 规划裕量，按 `0.35 m` 有效半径检查节点和边；从 `(0,0,1.5)` 到 `(12,2.7,1.5)` 的确定性结果为 `51` 个路径点、长度 `14.624585 m`、扩展 `4862` 个节点，最大高度 `1.6 m`；
- 默认原始路径可确定性简化为 `7` 个点、长度 `14.022856 m`；连续轨迹选择 `velocity_scale=1.00`，总时长 `40.065302 s`，最大速度 `0.548638 m/s`、最大加速度 `0.178899 m/s²`，`2005` 个验证采样及其相邻连线均通过 `0.35 m` 有效规划碰撞模型；
- 用户已在 RViz2 中人工确认黄色 A* 原始栅格路径、粉色视线简化折线和蓝色连续参考轨迹能够同时显示；蓝色连续参考轨迹整体平滑，符合当前路径轨迹化预期。视觉观察不用于精确测量安全距离，几何安全仍以自动碰撞验证为依据；
- Domain 99 静态避障执行回归中，无人机先在 `(0,0,1.5)` 稳定，再按 `0→1→2→3→4→5` 执行规划轨迹并到达 `(12,2.7,1.5)`；最大跟踪误差 `0.029911 m`、对基础膨胀障碍物的最小净空 `0.218112 m`、最终误差 `0.004557 m`、最终速度 `0.001606 m/s`，完成后继续保持至少 `3.0 s`，控制器日志未出现饱和；
...
- Domain 113 有序多目标回归从实际地面 Odom 原地起飞，依次到达 P1 `(4.2,2.5,1.5)`、P2 `(7.4,0,1.5)`、P3 `(12,2.7,1.5)`；三段分别展示第一道上绕、中部通道和末端上部通道。任务于 `67.690 s` 完成，三段最大跟踪误差为 `0.009384/0.010359/0.009494 m`，最小净空 `0.240352 m`，最终误差 `0.001466 m`、最终速度 `0.000684 m/s`；全程无碰撞、非有限值或控制饱和；
- 当前三目标任务已由 Domain 113 闭环回归确认严格按序执行；RViz2 保留多目标路径、实际轨迹和环境显示，当前五障碍地图的定量验收以自动测试和评测数据为准；
- RViz2 配置已将 Orbit 焦点移至地图中心并把观察距离扩大到 `16 m`，可同时显示完整工作空间、五个原始障碍物、透明安全膨胀区、无人机及四类路径；
...
环境配置位于 `src/drone_bringup/config/environment.yaml`。默认 workspace 为 `[-1,13] × [-2.5,6.5] × [-0.5,5] m`，五个全高墙段组成上绕、中部通道、上部通道三道屏障；默认 A* 契约为 `(0,0,1.5) → (12,2.7,1.5)`，分辨率保持 `0.25 m`。`/drone/environment/markers` 显示工作空间、原始障碍物和透明安全膨胀区；`/drone/environment/in_collision` 只在收到有限且未超时的 Odom 时发布。无图形界面时可增加 `use_rviz:=false`。
...
默认任务配置 `multi_goal_mission.yaml` 依次包含 `(4.2,2.5,1.5)`、`(7.4,0,1.5)` 和 `(12,2.7,1.5)` 三个零 yaw 目标；可用 `mission_config:=<绝对路径>` 载入独立任务 YAML。节点启动时一次性读取目标列表，以首次有效 Odom 的 x/y 为起飞锚点，先发布 `z=1.5 m` 的静止 setpoint；位置与速度连续稳定 `1.0 s` 后，才从当时实际 Odom 规划 P1。每个目标同样采用“到达、停稳、切换”语义，Odom 无效或超时会暂停当前轨迹时钟，最终持续保持最后目标。任务状态可通过 `/drone/multi_goal/current_goal_index`、`current_segment`、`complete`、`success` 和 `visited_goals` 五个 Topic 观察。
