...
本轮没有加入位置或高度积分项，以隔离空气动力学和执行包线的影响。下一阶段若要改善持续恒力偏差，应单独评估带 anti-windup 的位置积分，而不是修改安全半径、提高轨迹速度或继续扩大执行器参数。模型仍未包含旋翼陀螺效应、桨叶挥舞、地面效应、电池电压下降、电机非对称、传感器噪声、复杂阵风和空气密度随高度变化。
