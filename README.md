...
核心算法与 ROS2 通信层分离，动力学、控制器、Motor Mixer 和多目标点任务管理均可独立测试。项目最终目标还包括平滑轨迹、地图、路径规划和静态避障。

## 当前阶段

动力学、高度/yaw、单目标三维位置闭环和第一版多目标点顺序飞行均已完成并通过真实运行验收。`WaypointManager` 依次发布 `/drone/goal`，只有位置、线速度、yaw 和角速度连续满足阈值 `1.0 s` 才切换到下一点；任务结束后继续保持最终目标。
...
- `basic_sim.launch.py` 一键启动动力学、控制器、机器人模型发布和 RViz2；`mission_sim.launch.py` 在此基础上启动顺序任务节点。
...
- 当前工作区最近一次完整测试结果为 `123 tests, 0 errors, 0 failures, 0 skipped`。
...
- `drone_mission`：与 ROS2 无关的顺序 WaypointManager、任务节点及单元测试；
...
- 多目标点当前采用“到点并稳定后离散切换”的第一版策略，尚未实现平滑轨迹生成与跟踪；
- 地面模型只有质心 z 方向的无反弹、无摩擦刚性约束；
- 动力学暂不包含空气阻力、旋翼陀螺效应和传感器噪声；
- 当前控制器没有位置积分环和复杂反饱和机制；
- 多目标任务已实际启动 RViz2 并完成数值监测，但飞行动画效果尚未由用户直接人工确认；
