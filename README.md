...
专用 RViz 配置预先打开 RobotModel、TF、Odom、绿色实际轨迹、悬停目标和 `/drone/disturbance/markers`。红色箭头是作用于质心的 **Equivalent External Force**，长度与 N 制等效集中外力成比例；蓝色箭头仅是水平位置积分产生的加速度补偿，单位语义为 `m/s²`，不是实际作用力。黄色细线连接实际位置和目标。上方文字按阶段精简为 1～3 行：只保留当前动作、必要的力/误差/积分和剩余时间；正常不显示全零饱和状态，只有发生饱和时才追加一行 WARNING。零外力和近零积分时相应箭头会删除，不会构造零向量方向。
...
正常演示不要求执行全部诊断命令。按 A→I 完成人工验收后只执行一次 J；不同仿真 Launch 不要同时运行在同一 ROS Domain。

### A. 编译与环境

测试目的：确认工作区可构建且本地 ROS 包可发现。
...
通过标准：构建退出码为 0，列表包含 `drone_bringup`、`drone_controller`、`drone_dynamics`、`drone_mission`、`drone_msgs`、`drone_planning`。
...
必要操作：等待无人机起飞并稳定；结束时在 Launch 终端按 `Ctrl+C`。

RViz观察重点：无人机起飞到 `z=1.5 m`，水平位置稳定，无明显振荡，不显示扰动箭头。

通过标准：能够稳定悬停；无 NaN/Inf、明显姿态发散或持续控制饱和。
...
测试目的：验证 `map` 坐标系中目标 `(2.0,1.0,1.5)`、`yaw=0` 的三维位置控制。
...
必要操作：先启动仿真，再从第二终端发布一次目标；结束时在 Launch 终端按 `Ctrl+C`。

RViz观察重点：无人机从当前位置向 `(2,1,1.5)` 正确运动，最终在目标附近稳定悬停，无持续振荡或漂移，不出现扰动箭头。

通过标准：能够到达目标附近并稳定悬停；无 NaN/Inf、明显姿态发散或持续控制饱和。
...
测试目的：复用正式 `mission_sim.launch.py`、`mission.yaml` 和 `WaypointManager`，验证 stop-settle-switch 多目标顺序飞行。
...
cd /home/peter/ros2_drone_sim
source install/setup.bash
ros2 launch drone_bringup mission_sim.launch.py use_rviz:=true
```

必要操作：无需手工发布目标；等待任务完成后按 `Ctrl+C`。现有任务在 XY 平面访问四个不同角点并返回起点，形成矩形闭环航线，同时包含高度和 yaw 变化。

RViz观察重点：无人机严格按序访问目标，每到目标附近停稳后再切换，不跳过目标；最终返回起飞区域悬停，实际轨迹形成闭合航线；场景中没有障碍物或主动外力。

通过标准：目标严格按序完成；每个目标附近能够停稳；任务最终完成；无明显发散和控制饱和。

可选诊断：

```bash
ros2 topic echo /drone/mission/current_waypoint_index
ros2 topic echo /drone/mission/complete
```
...
必要操作：等待 TAKEOFF/SETTLING、倒计时、扰动、恢复和完成阶段自动运行；结束时按 `Ctrl+C`。

RViz观察重点：倒计时文字简洁；扰动期间红色 Equivalent External Force 箭头沿 `+X` 出现，无人机同向偏移，蓝色积分加速度箭头逐渐出现；撤力后进入 RECOVERY，红箭头删除并最终返回目标附近。

通过标准：阶段、箭头和飞行运动一致；恢复完成，无残余红箭头、NaN/Inf 或持续控制饱和。
...
必要操作：等待持续扰动和撤力恢复自动完成；结束时按 `Ctrl+C`。

RViz观察重点：红色外力箭头保持 10 s，蓝色积分补偿总体指向扰力反方向；持续扰动时偏差逐渐减小，撤力后蓝色箭头逐渐缩短，无严重反向冲击，最后接近目标。

通过标准：补偿方向正确，撤力恢复平顺，最终误差收敛且无持续控制饱和或非有限值。
...
必要操作：无需手工发布目标；等待 P1→P2→P3 完成，结束时按 `Ctrl+C`。

RViz观察重点：地图、原始障碍物、透明膨胀区、目标、蓝色参考路径和绿色实际轨迹均可见；无人机按序飞行且不穿障；不显示扰动状态。

通过标准：全部目标完成，无碰撞、主动外力、NaN/Inf 或持续控制饱和。
...
必要操作：选择工具栏 `Interact`；拖动 `goal_candidate` 的水平面和竖直箭头；右键依次选择 `Add Goal`、`Validate & Preview`，READY 且蓝色预览完整后选择 `Execute Validated Mission`；结束时按 `Ctrl+C`。

RViz观察重点：安全候选为绿色；障碍物内部和 workspace 外候选为红色并拒绝；执行中观察蓝色参考路径、绿色实际轨迹和 Pn 状态。

通过标准：至少一组安全目标完成；障碍物内部和 workspace 外目标均被拒绝；无路径时保持安全。
...
通过标准：测试无 failure/error；日志明确报告 preflight 无路径失败；有效 trajectory setpoint 数为 0、RPM 始终为 0、无人机不离地且无水平移动。

可选诊断：失败时查看 `build/drone_bringup/Testing/Temporary/LastTest.log`。

### J. 最终自动全量测试

测试目的：仅在 A～I 全部通过后执行合并 `main` 前的最终回归。
...
必要操作：只执行一次，并记录 tests、failures、errors、skipped。

RViz观察重点：本项不启动 RViz。

通过标准：`failures=0`、`errors=0`。

可选诊断：失败时使用 `colcon test-result --all --verbose` 定位具体用例。
...
本附录是可选深度诊断，不属于日常演示必做流程。只有视觉异常、需要正式记录内部状态或合并 `main` 前深度核对时才执行。
