...
- 默认场景的三维 26 邻域 A* 使用 `0.25 m` 基础安全半径和 `0.10 m` 规划裕量，按 `0.35 m` 有效半径检查节点和边；默认单目标为 `(0,0,1.5) → (12.1,1.1,1.5)`；
- 额外高 y 目标 A `(12.1,5.5,1.5)` 的正式评测得到 68 个原始路径点、`19.981888 m`，扩展 17255 个节点；简化为 9 点、`18.873374 m`，连续轨迹 `54.326734 s`，所有采样点与相邻线段均通过 `0.35 m` 有效规划碰撞检查；
- 用户已在 RViz2 中人工确认黄色 A* 原始栅格路径、粉色视线简化折线和蓝色连续参考轨迹能够同时显示；蓝色连续参考轨迹整体平滑，符合当前路径轨迹化预期。视觉观察不用于精确测量安全距离，几何安全仍以自动碰撞验证为依据；
- 目标 A 闭环评测于 `59.907 s` 完成，最大跟踪误差 `0.030705 m`、对基础膨胀障碍物的最小净空 `0.183027 m`、最终误差 `0.005585 m`、最终速度 `0.001465 m/s`；全程无碰撞、非有限值或控制器饱和；
- 评测场景 B `(12.1,1.1,2.5)` 与 C `(12.1,1.1,3.25)` 也完成闭环；B/C 最大跟踪误差为 `0.030707/0.030107 m`、最小净空为 `0.170592/0.171083 m`，均无碰撞、非有限值或饱和；
...
- 默认单目标闭环由 55 个原始路径点简化为 7 点、6 段，总轨迹 `43.926338 s`，任务于 `48.264 s` 完成；最大跟踪误差 `0.031146 m`、最小基础净空 `0.170603 m`、最终误差 `0.002673 m`，无碰撞或饱和；
- Domain 113 有序多目标回归从实际地面 Odom 原地起飞，依次执行远端 P1 `(12.1,1.1,1.5)`、中部上侧高点 P2 `(7.0,5.0,4.0)` 和起飞区偏置 P3 `(0.8,0.7,2.0)`；任务于 `144.863 s` 完成，三段最大跟踪误差为 `0.009225/0.018765/0.016794 m`，最小净空 `0.135292 m`，最终误差 `0.002819 m`、最终速度 `0.001169 m/s`，无碰撞、非有限值或饱和；
...
- 当前完整测试结果为 `211 tests, 0 errors, 0 failures, 0 skipped`。
...
环境配置位于 `src/drone_bringup/config/environment.yaml`。默认 workspace 为 `[-1,14.5] × [-2.5,7.0] × [-0.5,5] m`，三个在安全膨胀后连接边界的高墙与三个内部独立高障碍物形成水平蛇形路线和局部挑战通道；默认 A* 契约为 `(0,0,1.5) → (12.1,1.1,1.5)`，分辨率保持 `0.25 m`。`/drone/environment/markers` 显示工作空间、原始障碍物和透明基础安全膨胀区；`/drone/environment/in_collision` 只在收到有限且未超时的 Odom 时发布。无图形界面时可增加 `use_rviz:=false`。
...
默认任务配置 `multi_goal_mission.yaml` 依次包含 `(12.1,1.1,1.5)`、`(7.0,5.0,4.0)` 和 `(0.8,0.7,2.0)` 三个零 yaw 目标：P1 完整穿越地图到远端，P2 从高空返回中部上侧，P3 偏置返航到起飞区附近。末端空间扩展后，额外目标 A `(12.1,5.5,1.5)` 已通过 A*、简化路径、连续轨迹和闭环飞行验证，但本轮没有直接替换默认 P1。当前节点显式要求 yaw 为零。可用 `mission_config:=<绝对路径>` 载入独立任务 YAML。节点启动时一次性读取目标列表，以首次有效 Odom 的 x/y 为起飞锚点，先发布 `z=1.5 m` 的静止 setpoint；位置与速度连续稳定 `1.0 s` 后，才从当时实际 Odom 规划 P1。每个目标同样采用“到达、停稳、切换”语义，Odom 无效或超时会暂停当前轨迹时钟，最终持续保持最后目标。任务状态可通过 `/drone/multi_goal/current_goal_index`、`current_segment`、`complete`、`success` 和 `visited_goals` 五个 Topic 观察。
