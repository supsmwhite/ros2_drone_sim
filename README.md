...
- Domain 113 有序多目标回归从实际地面 Odom 原地起飞，首先直接规划到远端 P1 `(12.1,1.1,1.5)`，再返航到中部上侧高点 P2 `(7.0,5.0,4.0)`，最后到达起点附近但不重合的 P3 `(0.8,0.7,2.0)`。任务于 `144.898 s` 完成，各段最大跟踪误差为 `0.012698/0.018786/0.016702 m`，最小净空 `0.135290 m`，最终误差 `0.002820 m`、最终速度 `0.001170 m/s`；全程无碰撞、非有限值或控制饱和；
- 当前远程首目标与两段偏置返航任务已由 Domain 113 闭环回归确认严格按序执行；任务结束时绿色实际 Path 仍保留从起飞点开始的 `1479` 个历史点，当前六障碍地图的定量验收以自动测试为准；
...
默认任务配置 `multi_goal_mission.yaml` 依次包含 `(12.1,1.1,1.5)`、`(7.0,5.0,4.0)` 和 `(0.8,0.7,2.0)` 三个零 yaw 目标：先直接进行跨地图远距离导航，再经过中部上侧高点返航，最终回到起飞区附近的偏置位置。用户提出的 `(12.1,5.5,1.5)` 与 `(7.0,6.0,4.0)` 虽然点和 A* 路径合法，但连续轨迹安全验证失败，故分别向安全通道和工作空间内部调整。当前节点显式要求 yaw 为零，因此未填写无效航向变化。可用 `mission_config:=<绝对路径>` 载入独立任务 YAML。节点启动时一次性读取目标列表，以首次有效 Odom 的 x/y 为起飞锚点，先发布 `z=1.5 m` 的静止 setpoint；位置与速度连续稳定 `1.0 s` 后，才从当时实际 Odom 规划 P1。每个目标同样采用“到达、停稳、切换”语义，Odom 无效或超时会暂停当前轨迹时钟，最终持续保持最后目标。任务状态可通过 `/drone/multi_goal/current_goal_index`、`current_segment`、`complete`、`success` 和 `visited_goals` 五个 Topic 观察。
