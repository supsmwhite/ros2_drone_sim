...
核心算法与 ROS2 通信层分离，动力学、控制器、Motor Mixer、多目标点任务管理、分段五次轨迹、静态环境碰撞检查、三维栅格 A*、视线简化和安全规划轨迹生成均可独立测试。下一阶段目标是规划轨迹自动执行与静态避障端到端验收。

## 当前阶段

动力学、高度/yaw、单目标三维位置闭环、第一版多目标点顺序飞行、连续轨迹生成与跟踪、静态三维 AABB 环境、统一碰撞查询、三维 26 邻域 A*、确定性视线简化和经过验证的规划轨迹生成均已完成。`planned_trajectory_sim.launch.py` 可同时显示原始 A* 路径、简化折线和连续参考轨迹；规划轨迹仍未接入控制器，不会介入动力学或电机控制。
...
该链路不发布 `/drone/trajectory_setpoint`。
...
- 当前工作区最近一次完整测试结果为 `199 tests, 0 errors, 0 failures, 0 skipped`。
...
`/drone/path` 是动力学实际状态的历史位姿；`/drone/planned_path` 是 A* 原始栅格路径；`/drone/simplified_path` 是视线简化折线；`/drone/reference_path` 是连续参考轨迹。四者用途不同，规划轨迹目前不会驱动无人机。
...
规划轨迹配置位于 `src/drone_bringup/config/planned_trajectory.yaml`，环境几何与安全参数继续来自 `environment.yaml`，规划裕量继续来自 `astar.yaml`，没有在新配置中复制。节点只处理收到的第一条合法原始路径；默认以 `0.35 m/s` 名义速度生成各段时间，依次尝试 `[1.0, 0.75, 0.5, 0.25, 0.0]`，并以 `0.02 s` 周期验证速度、加速度和 `0.35 m` 有效半径下的碰撞安全。`0.35 m/s` 是在保持明确分段时间公式和 `0.35 m/s²` 加速度上限不变时的默认可行值；`0.45 m/s` 在默认简化路径上实测峰值加速度约 `0.573 m/s²`，无法通过同一验收约束。节点发布 `/drone/simplified_path`、`/drone/reference_path` 和集中指标，不发布 `/drone/trajectory_setpoint`。
...
- 当前静态环境、A*、路径简化和规划轨迹只提供几何查询、全局结果、验证与可视化，不会阻止电机、改变动力学或产生物理撞击响应；
- 尚未实现规划轨迹自动执行、在线重规划或动态障碍物；
