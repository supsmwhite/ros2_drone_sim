...
`PlannedTrajectoryBuilder` 的首个候选仍使用原简化路径、原分段时长和原 `velocity_scale` 候选。仅当完整采样验证报告点或线段碰撞时，才在失败轨迹段附近按原始 A* 索引确定性恢复局部点；最多细化 8 轮、每轮最多插入 3 点。若几何已安全但仅速度或加速度超限，则依次尝试 `duration_scale=[1.0,1.25,1.5,2.0,3.0,4.0]`。所有候选均重新验证有限值、端点、速度、加速度、采样点与相邻采样线段；实现不包含目标坐标或障碍物编号硬编码。

远端 P1 最终使用 3 轮细化、18 个 waypoint、`velocity_scale=1.0`、`duration_scale=1.0`。参考轨迹对 `0.35 m` 规划膨胀模型的采样最小净空为 `0.050000 m`。安全半径、规划裕量、地图、A* 核心算法和名义速度均未改变。
...
- 总任务时间：`183.864 s`；
- 三段最大跟踪误差：`0.015722/0.017182/0.018736 m`；
- 三段最小基础净空：`0.108158/0.137670/0.200972 m`，全局最小 `0.108158 m`；
- 最终误差：`0.002867 m`；最终速度：`0.001180 m/s`；
- 实际轨迹历史：1869 点，任务结束时仍包含起飞初始点；
- 无碰撞、非有限值或控制器饱和。
...
- 远端 P1 需要 3 轮局部路径细化，实际最小基础净空 `0.089736 m`，后续提速时必须重点复验该段。
- 完整多目标任务约 `184 s`，人工演示应预留至少 4 分钟并保持绿色实际轨迹显示开启。
