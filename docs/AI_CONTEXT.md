...
| 目标 | 坐标 `(x,y,z,yaw)` | 作用 |
|---|---|---|
| P1 | `(12.1,1.1,1.5,0.0)` | 远端完整穿越六障碍地图 |
...
A、默认 P1、P2、P3 均在 `0.35 m` 模型下安全且 A* 可达。A 已通过简化路径、连续轨迹和闭环飞行验证，但未替换默认多目标 P1，以保持地图修改与正式任务变更解耦。

多目标轨迹名义速度保持 `0.25 m/s`；单目标轨迹名义速度保持 `0.35 m/s`。控制器增益、最大倾角、最大水平加速度、最大参考加速度、最大 RPM、A* 分辨率和核心算法均未因地图修改而改变。
...
| A* 成功 | 是 |
| 扩展节点 | `17255` |
| 原始路径 | `68` 点 / `19.981888 m` |
| 简化路径 | `9` 点 / `18.873374 m` |
| 轨迹时间 | `54.326734 s` |
| 速度比例 | `1.00` |
| 最大参考速度 | `0.552505 m/s` |
| 最大参考加速度 | `0.321714 m/s²` |
| 闭环任务时间 | `59.906892 s` |
| 最大跟踪误差 | `0.030705 m` |
| 对基础膨胀障碍物最小净空 | `0.183027 m` |
| 最终位置误差 | `0.005585 m` |
| 最终速度 | `0.001465 m/s` |
| 碰撞 / NaN/Inf / 饱和 | 无 / 无 / 无 |
...
|---|---|---:|---:|---:|---:|---:|---:|
| B 水平高度 | `(12.1,1.1,2.5)` | 55 点 / `16.490537 m` | 7 点 / `15.358924 m` | `44.285448 s` | `0.030707 m` | `0.170592 m` | `0.002634 m` |
| C 三维高度 | `(12.1,1.1,3.25)` | 55 点 / `16.728915 m` | 7 点 / `15.495720 m` | `44.676295 s` | `0.030107 m` | `0.171083 m` | `0.002680 m` |

B/C 均无碰撞、非有限值或控制器饱和。C 的 y 坐标与当前安全末端线 `1.1 m` 对齐，继续保留高度变化评测用途。

## 默认任务闭环验证

默认单目标 `(12.1,1.1,1.5)`：

- A*：55 点、`16.172700 m`、扩展 8087 节点；
- 简化路径：7 点、`15.233235 m`；
- 连续轨迹：6 段、`43.926338 s`、速度比例 `1.00`；
- 闭环：`48.264 s` 完成，最大跟踪误差 `0.031146 m`，最小基础净空 `0.170603 m`，最终误差 `0.002673 m`、最终速度 `0.001086 m/s`；
- 无碰撞、非有限值或控制器饱和。

默认多目标：

- 总任务时间：`144.863 s`；
- 三段最大跟踪误差：`0.009225/0.018765/0.016794 m`；
- 最小基础净空：`0.135292 m`；
- 最终误差：`0.002819 m`；最终速度：`0.001169 m/s`；
- 实际轨迹历史：1479 点，任务结束时仍包含起飞初始点；
- 无碰撞、非有限值或控制器饱和。
...
- 绿色 `/drone/path`：实际 Odom 历史轨迹，10 Hz，最多 6000 点，约保留 10 分钟
- 黄色 `/drone/planned_path`：A* 原始路径
- 粉色 `/drone/simplified_path`：简化折线
- 蓝色 `/drone/reference_path`：连续参考轨迹
...
最近一次结果：`211 tests, 0 errors, 0 failures, 0 skipped`。其中 `drone_bringup` 的 8 个 Launch 测试全部通过；目标 A/B/C 的较长评测工具运行也全部通过。

目标 A 闭环评测：

```bash
python3 tools/evaluate_static_avoidance.py \
  --scenario scenario_a_default \
  --output-root /tmp/terminal_reachability_eval \
  --timeout 90
```
