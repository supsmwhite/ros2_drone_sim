...
项目采用“纯算法类与 ROS2 节点分离”的结构。动力学、三维控制、顺序多目标点、C² 连续轨迹、静态三维 AABB 环境、统一碰撞查询、三维 26 邻域 A*、确定性视线简化、安全规划轨迹生成、静态避障执行、三场景系统评测和有序多目标静态避障均已实现，并有可重复的 ROS2 集成回归与实验数据曲线。

## 2. 当前阶段和下一任务

### 当前阶段

动力学、三维控制、顺序多目标点任务、连续轨迹跟踪、静态环境碰撞检查、三维栅格 A*、路径简化、规划轨迹验证、静态避障执行和有序多目标静态避障均已完成。三个独立目标场景已通过完整闭环评测并生成 JSON、CSV 和四类曲线；新的多目标任务从实际地面 Odom 起飞，在 `0.50 m` 导航地板以上逐段规划，并以 stop-settle-switch 语义严格按序执行。
...
- 最终验收 workspace 为 `[-1,13] × [-2.5,6.5] × [-0.5,5] m`，五个全高墙段组成三道屏障：先上绕、再穿 `1.5 m` 有效中部通道、最后穿 `1.8 m` 有效上部通道；安全半径保持 `0.25 m`。
- 静态避障场景契约是 start `(0,0,1.5)`、goal `(12,2.7,1.5)`、栅格 `0.25 m`：起终点安全、直线受阻且不能从同一外围绕过全部屏障；真实闭环执行已通过 Domain 99 验收。
...
- 默认 `(0,0,1.5) → (12,2.7,1.5)` 原始规划结果通过 `/drone/planned_path` transient-local 发布；`planned_trajectory_node` 以相同 `0.35 m` 有效规划半径生成简化路径和连续参考。默认 `execution_enabled=false` 不发布 setpoint；启用后先准备起点，再通过 `/drone/trajectory_setpoint` 驱动真实闭环，Odom 无效或超时时暂停轨迹时钟，完成后保持终点。
- `multi_goal_static_avoidance_node` 读取第一个有效 Odom，以其实际 x/y 为起飞锚点并单独检查到 `z=1.5 m` 的竖直段；起飞稳定后，每一段都从当时稳定的实际 Odom 规划到当前目标，目标稳定保持 `1.0 s` 后才推进索引。
- 多目标空中规划使用独立导航环境：原始 workspace 最低 z 从 `-0.5 m` 调整为 `0.15 m`，再由 `0.35 m` 有效规划半径收缩为严格安全最低高度 `0.50 m`；共享环境、动力学地面、障碍物、`0.25 m` 基础安全半径和 `0.10 m` 规划裕量不变。
- 多目标任务配置使用 `nominal_speed=0.25 m/s`、`max_reference_acceleration=0.35 m/s²`。实际首段从地面起飞后的 Odom 到 P1 时，`0.35` 与 `0.30 m/s` 均无法通过未改变的加速度约束，`0.25 m/s` 可行；原有单目标 `planned_trajectory.yaml` 仍为 `0.35 m/s`。
...
局部规划、动态障碍或在线重规划继续作为可选扩展，不是当前主线必做内容；若后续开展，应作为独立阶段，不改变当前静态全局轨迹执行与评测基线。
...
首次有效 Odom
→ 原地竖直起飞并稳定到 z=1.5 m
→ 当前实际 Odom → AStarPlanner → PathSimplifier → PlannedTrajectoryBuilder
→ /drone/trajectory_setpoint → PositionController → Mixer → 动力学
→ 当前目标稳定保持 → 从新的实际 Odom 规划下一目标
```

该链路由 `multi_goal_static_avoidance_node` 独占发布三种规划 Path 和 trajectory setpoint，不同时启动 `astar_planner_node`、`planned_trajectory_node`、`trajectory_mission_node` 或 `waypoint_manager_node`。规划在线程任务中进行，节点在规划期间仍以 `50 Hz` 保持安全静止 setpoint；Odom 失效或超过 `0.25 s` 时暂停轨迹时钟。
...
- `multi_goal_static_avoidance_node`：直接组合 `AStarPlanner`、`PathSimplifier` 和 `PlannedTrajectoryBuilder`；负责首次 Odom 起飞、竖直段检查、导航地板、逐目标异步规划/执行、目标停稳切换、超时暂停、失败安全保持和任务状态发布。
...
- `static_avoidance_sim.launch.py` 不复用固定五点轨迹 Launch，直接启动动力学、轨迹模式控制器、robot_state_publisher、可选 RViz2、环境、A* 和执行模式规划轨迹节点；可选 `astar_config` 默认仍为 `astar.yaml`，评测时可载入独立场景 YAML；
- `tools/evaluate_static_avoidance.py` 在独立 Domain `110/111/112` 中顺序运行 A/B/C 三个完整场景，独立启动和关闭 Launch，检查实际 Odom 点和线段安全并输出结构化指标、轨迹数据和 matplotlib 曲线；该长时间工作流不加入默认 `colcon test`；
- `multi_goal_static_avoidance_sim.launch.py` 只启动动力学、轨迹模式控制器、robot_state_publisher、可选 RViz2、静态环境和多目标任务节点；`mission_config` 默认指向远程首目标与两段返航的三目标 `multi_goal_mission.yaml`；
...
src/drone_planning/src/multi_goal_static_avoidance_node.cpp
...
src/drone_bringup/config/astar_evaluation_scenario_a.yaml
src/drone_bringup/config/astar_evaluation_scenario_b.yaml
src/drone_bringup/config/astar_evaluation_scenario_c.yaml
src/drone_bringup/config/planned_trajectory.yaml
src/drone_bringup/config/multi_goal_mission.yaml
src/drone_bringup/config/multi_goal_mission_test.yaml
...
src/drone_bringup/launch/multi_goal_static_avoidance_sim.launch.py
...
src/drone_bringup/test/test_static_avoidance_e2e.py
src/drone_bringup/test/test_multi_goal_static_avoidance_e2e.py
src/drone_bringup/urdf/drone.urdf.xacro
src/drone_bringup/rviz/drone_sim.rviz

tools/evaluate_static_avoidance.py
results/static_avoidance/
```
...
| Path | 名义 `10 Hz`，最多 `6000` 点（约 10 分钟历史） |
...
- 三个静态避障评测场景、独立目标 YAML、顺序评测工具、JSON/CSV 和数据曲线；
- 首次实际 Odom 起飞、导航地板约束和逐段重规划的有序多目标静态避障任务；
- MotorRPM watchdog；
- 当前文档整理结果。

`main` 已包含静态避障执行和三场景系统评测。本分支增加有序多目标静态避障任务节点、专用配置、Launch 和回归；复用既有规划/轨迹纯算法，不改变控制器、动力学、共享环境、基础安全半径、规划裕量或 A* 分辨率，也不实现局部规划、动态障碍或在线重规划。Git 当前状态始终以实际命令为准。
...
- 当前工作区完整汇总为 `209 tests, 0 errors, 0 failures, 0 skipped`；
...
- `CollisionChecker` 有 23 项确定性纯算法测试，覆盖环境/半径合法性、工作空间和障碍物边界、安全点、膨胀区、非有限输入，三维线段穿越、相切、角点、平行、端点、零长度和极短线段，以及默认规划演示场景的直线受阻、已知安全折线和三个评测目标的现有碰撞模型合法性；
...
- `PlannedTrajectoryBuilder` 测试除原有默认场景、安全与动态约束、精确首尾、确定性、候选下降与非法参数外，还覆盖 `0.50 m` 导航地板、首次实际起飞 Odom 和默认远程首目标/偏置返航全部有序段；
...
- `test_multi_goal_static_avoidance_e2e.py` 使用独立 `ROS_DOMAIN_ID=113`，真实执行远程 P1 与两个返航目标 P2/P3，检查首次实际 Odom 起飞、索引与已访问计数严格顺序、每段规划起点来自稳定实际 Odom、三类 Path 数量和导航地板、逐目标停止验收、最终保持、发布者/节点独占、消息活性，以及独立 Odom 点/闭 slab 连线碰撞、跟踪误差和净空；
...
- 静态环境集成运行已确认 `workspace/obstacles/inflated_obstacles` Marker 分类、碰撞 true/false 转换，以及非法或超时 Odom 不发布状态；RViz Orbit 焦点为 `(6,2,1.5)`、距离 `16 m`，六个原始障碍物、膨胀区和完整任务路径可处于同一默认视野；
- 默认 A* ROS2 集成使用 `0.35 m` 有效规划半径，输出 `55` 个路径点、总长 `16.069147 m`、扩展约 `7680` 个节点、最大高度 `1.6 m`；
- 默认规划轨迹集成将 `55` 个原始点简化为 `8` 个点，折线长度降为 `15.216305 m`；选择 `velocity_scale=1.00`，总时长 `43.877966 s`，最大速度 `0.552505 m/s`、最大加速度 `0.321714 m/s²`，独立几何复核无轨迹碰撞；
...
- Domain 99 静态避障完整回归中，起点准备后按段索引 `0→1→2→3→4→5→6` 执行，任务于 `48.194 s` 完成；最大跟踪误差 `0.029918 m`，实际 Odom 对基础膨胀障碍物的最小净空 `0.183089 m`，控制器日志 `saturated=true` 为 `0` 次；最终误差 `0.005209 m`、速度 `0.001550 m/s`，全程无碰撞或 NaN/Inf；
- 用户已在 RViz2 中观察静态避障完整执行，整体运动和绕障效果符合预期。精确碰撞净空、跟踪误差和最终误差仍以自动回归指标为准；
- 静态避障系统评测使用三个独立规划 YAML 和 ROS Domain `110/111/112`，起点均为 `(0,0,1.5)`；三个目标先由现有 `CollisionChecker` 单元测试确认安全，随后均通过真实完整闭环验收：

| 场景 | goal | 原始路径（点 / m） | 简化路径（点 / m） | 轨迹（s / scale） | 参考峰值（m/s / m/s²） | 最大跟踪误差 / 最小净空（m） | 最终误差 / 速度（m / m/s） |
|---|---|---:|---:|---:|---:|---:|---:|
| A 默认蛇形 | `(12.1,0.85,1.5)` | `55 / 16.069147` | `8 / 15.216305` | `43.877966 / 1.00` | `0.552505 / 0.321714` | `0.031314 / 0.183008` | `0.005296 / 0.001594` |
| B 水平终点 | `(12.1,1.1,2.5)` | `55 / 16.490537` | `8 / 15.382273` | `44.352159 / 1.00` | `0.553908 / 0.321644` | `0.030909 / 0.178332` | `0.004844 / 0.001441` |
| C 三维高度 | `(12.1,0.85,3.25)` | `55 / 16.625362` | `8 / 15.478790` | `44.627923 / 1.00` | `0.553908 / 0.324166` | `0.030107 / 0.181431` | `0.005302 / 0.001586` |

三个场景的环境碰撞状态均未出现 true，实际 Odom 点和连续线段均未进入基础 `0.25 m` 膨胀障碍物，无 NaN/Inf，控制器日志 `saturated=true` 均为 `0` 次。场景 A/B/C 分别采集 Odom `7784/7596/7245`、setpoint `1925/1888/1801`、RPM `3892/3798/3622` 个样本。每个场景的 `metrics.json`、`trajectory.csv`、`xy_path.png`、`position_tracking.png`、`tracking_error.png` 和 `clearance.png` 位于 `results/static_avoidance/<scenario_name>/`；
- Domain 113 有序多目标真实闭环依次执行远端 P1 `(12.1,1.1,1.5)`、中部上侧高点 P2 `(7.0,5.0,4.0)`、起飞区偏置返航点 P3 `(0.8,0.7,2.0)`，任务于 `144.898 s` 完成；三段最大跟踪误差 `0.012698/0.018786/0.016702 m`，最小净空 `0.135290 m`，最终误差 `0.002820 m`、速度 `0.001170 m/s`；共采集 Odom/setpoint/RPM/碰撞状态 `29573/7393/14787/2958` 个样本，完成后继续收到 `600/150/300` 个 Odom/setpoint/RPM 样本；绿色实际 Path 保留从起飞点开始的 `1479` 个历史点；环境碰撞未出现 true，独立 Odom 点和连续线段检查均安全，无 NaN/Inf，控制器日志 `saturated=true` 为 `0` 次，节点和最终 setpoint 持续存活；
- 当前远程首目标与偏置返航任务已由 Domain 113 闭环回归确认严格按序执行；RViz2 保留多目标路径、实际轨迹和环境显示，当前六障碍地图的定量验收以自动测试为准；
...
- 分支名称：`feature/five-obstacle-scene`
- 基准提交：`2278d9077ccb92d02098c0d193e62c74c707a998`（`main`）
- 当前提交：本节所在 `HEAD`；最终完整哈希以 `git rev-parse HEAD` 为准。Git 提交不能在自身内容中保存自己的最终哈希，否则内容变化会再次改变哈希。

### 本轮目标

- 扩大静态避障工作空间；
- 增加不少于 5 个真正影响默认路线的有效障碍物；
- 更新单目标与有序多目标任务；
- 同步 A* 配置、测试、评测工具、评测产物和 RViz2 默认视角；
- 不修改动力学、控制器增益、规划核心算法或标称速度。

### 实际修改

- 运行配置：`environment.yaml`、`astar.yaml`、三个 `astar_evaluation_scenario_*.yaml`、`multi_goal_mission.yaml`、`multi_goal_mission_test.yaml`；
- 可视化：`drone_sim.rviz`；
- ROS2 E2E：`test_astar_planner_e2e.py`、`test_planned_trajectory_e2e.py`、`test_static_avoidance_e2e.py`、`test_multi_goal_static_avoidance_e2e.py`；
- C++ 单元/组合测试：`test_astar_planner.cpp`、`test_collision_checker.cpp`、`test_planned_trajectory_builder.cpp`；
- 评测：`tools/evaluate_static_avoidance.py`，以及 `results/static_avoidance/` 下 A/B/C 三场景的 JSON、CSV、日志和曲线；
- 文档：`README.md` 与本文件。经审查，相关 Launch 已统一加载 `environment.yaml`，因此没有修改 Launch 结构。

最终 workspace 为 `[-1.0,13.0] × [-2.5,6.5] × [-0.5,5.0] m`。五个障碍物均为全高墙段：

| 障碍物 | 中心 `(x,y,z)` m | 尺寸 `(x,y,z)` m |
|---|---|---|
| O1 | `(2.8,-0.5,2.35)` | `(0.8,4.0,4.7)` |
| O2 | `(6.2,3.75,2.35)` | `(0.8,5.5,4.7)` |
| O3 | `(6.2,-1.85,2.35)` | `(0.8,1.3,4.7)` |
| O4 | `(9.6,-0.5,2.35)` | `(0.8,4.0,4.7)` |
| O5 | `(9.6,5.25,2.35)` | `(0.8,2.5,4.7)` |

三道屏障依次形成上绕、中部通道和上部通道；原始最窄通道为 `2.2 m`，按 `0.35 m` 有效规划半径膨胀后的最窄通道为 `1.5 m`。墙体膨胀顶部为 `5.05 m`，高于安全工作空间顶部 `4.65 m`，默认任务不能从上方飞越。

- 默认起点：`(0.0,0.0,1.5)`；
- 默认终点：`(12.0,2.7,1.5)`；
- 多目标：`(4.2,2.5,1.5) → (7.4,0.0,1.5) → (12.0,2.7,1.5)`；
- 基础安全半径：`0.25 m`；
- 规划附加裕量：`0.10 m`；
- 有效规划半径：`0.35 m`；
- A* 分辨率与节点上限保持原值 `0.25 m / 200000`；轨迹标称速度未修改。
...
```bash
python3 tools/evaluate_static_avoidance.py --timeout 90
```

- 构建结果：6 个 package 全部成功；
- 测试汇总：`209 tests, 0 errors, 0 failures, 0 skipped`；
- 默认 A*：成功，原始路径 `51` 点、`14.624585 m`，扩展 `4862` 节点；
- 默认连续轨迹：简化路径 `7` 点、`14.022856 m`，`velocity_scale=1.00`，总时长 `40.065302 s`；采样点和相邻线段均通过有效半径碰撞检查；
- Domain 99 单目标静态避障：任务于 `44.110 s` 完成，最大跟踪误差 `0.029911 m`，最小基础膨胀净空 `0.218112 m`，最终位置误差 `0.004557 m`，最终速度 `0.001606 m/s`；
- Domain 113 多目标静态避障：按索引 `0→1→2` 完成，任务时间 `67.690 s`，三段最大跟踪误差 `0.009384/0.010359/0.009494 m`，最小净空 `0.240352 m`，最终位置误差 `0.001466 m`，最终速度 `0.000684 m/s`；
- 独立 A/B/C 三场景评测全部通过；默认评测最大跟踪误差 `0.029521 m`、最小净空 `0.218269 m`、最终位置误差 `0.004415 m`；
- 所有上述闭环场景均未发生点或线段碰撞，未观察到 NaN/Inf，控制器日志中 `saturated=true` 次数为 `0`。

### 遗留问题

- 未通过测试：无；
- E2E Python 与 C++ 默认场景夹具仍硬编码新障碍物 AABB；评测工具已直接读取 `environment.yaml` 和对应 A* YAML，但测试夹具未来修改地图时仍需同步；
- 多目标完整演示约 `67.7 s`，比旧地图更长；当前 A* 扩展量 `4862`，未出现节点上限压力；
- RViz2 已调整默认焦点和距离，自动评测图确认五障碍及蛇形路线完整可见，但本轮没有新增交互式 GUI 自动验收；
- 最窄有效通道 `1.5 m` 在当前速度下有充足净空。后续速度优化必须重新执行连续轨迹和真实 Odom 点/线段碰撞检查，不得直接沿用本轮净空结论，也不得通过降低安全半径或放宽碰撞判定换取通过。

## 复杂障碍物与四目标三维任务优化

### 场景设计

工作空间保持 `[-1.0,13.0] × [-2.5,6.5] × [-0.5,5.0] m`，未通过扩大地图稀释难度。基础安全半径保持 `0.25 m`，规划裕量保持 `0.10 m`，有效规划半径仍为 `0.35 m`。六个互不重叠的全高障碍物如下：

| 障碍物 | 中心 `(x,y,z)` m | 尺寸 `(x,y,z)` m | 类型与作用 |
|---|---|---|---|
| O1 | `(2.6,-0.5,2.35)` | `(0.8,4.0,4.7)` | 连接下边界，迫使首段上绕 |
| O2 | `(4.6,4.15,2.35)` | `(0.8,4.7,4.7)` | 连接上边界，迫使路线换到下侧 |
| O3 | `(6.7,0.8,2.35)` | `(0.8,3.2,4.7)` | 内部障碍物，使路线再次向上换向 |
| O4 | `(8.9,3.75,2.35)` | `(0.8,5.5,4.7)` | 连接上边界，迫使后段下绕 |
| O5 | `(10.7,-0.85,2.35)` | `(0.8,1.3,4.7)` | 内部下侧障碍物，构成挑战通道 |
| O6 | `(10.7,2.95,2.35)` | `(0.8,2.5,4.7)` | 内部上侧障碍物，构成挑战通道 |

O1、O2、O4 是三个交替连接上下边界的高墙，阻止从同一外围一次绕过全部障碍物；O3、O5、O6 是内部独立障碍物。O5 与 O6 的原始表面间距为 `1.9 m`，按两侧各膨胀 `0.35 m` 后有效宽度为 `1.2 m`。所有障碍物顶部为 `4.7 m`，膨胀后高于 `4.65 m` 的安全工作空间上限，因此本轮没有低矮障碍物或三维越障；三维展示来自任务目标高度变化，不能统一升高飞越整张地图。

默认单目标起点/终点为 `(0.0,0.0,1.5) → (12.1,0.85,1.5)`。默认多目标任务实际读取四个导航目标：

| 目标 | `(x,y,z,yaw)` | 主要路线特征 |
|---|---|---|
| P1 | `(3.6,3.1,1.5,0.0)` | 从 O1 上侧绕行，到达第一障碍区后方 |
| P2 | `(7.85,3.1,2.5,0.0)` | 从 O2 下侧绕行，再绕 O3 上侧，形成连续水平换向并爬升 `1.0 m` |
| P3 | `(9.85,0.6,3.25,0.0)` | 从 O4 下侧绕行并继续爬升 `0.75 m` |
| P4 | `(12.1,0.85,1.5,0.0)` | 穿过 O5/O6 的 `1.2 m` 有效挑战通道并下降 `1.75 m` |

四个目标包含 `1.5/2.5/3.25 m` 三种高度，后三个任务段均有明确高度变化。当前任务节点在参数校验中显式拒绝非零 yaw，因此四个目标保持零 yaw，没有写入无法执行的航向要求。多目标 `nominal_speed=0.25 m/s`、最大参考加速度、停稳时间、控制频率、控制器与动力学参数均未修改。

### 同步修改

- 配置：统一更新 `environment.yaml`、默认和 A/B/C 三个 A* YAML、运行/测试多目标任务 YAML；
- 测试：同步更新四个 Python E2E 与三个 C++ 规划测试中的目标数量、顺序、六障碍 AABB 和挑战通道断言；
- 评测：`evaluate_static_avoidance.py` 已低风险复用 `environment.yaml`，无需复制障碍物；重新生成 A/B/C 的 JSON、CSV、日志和曲线；
- Launch：审查确认单目标、多目标、环境、规划和规划轨迹入口继续加载统一 `environment.yaml`，无需修改 Launch；
- RViz2：工作空间尺寸未变，现有 Orbit 焦点 `(6,2,1.5)`、距离 `16 m` 仍能显示完整地图；实际启动 RViz2 完整执行四目标任务并检查六障碍、无人机及路径显示，因此未做无必要视角改动；
- 核心算法：未修改动力学、控制器、A*、碰撞检测、路径简化、五次轨迹或 ROS 接口。
...
source install/setup.bash
colcon test --packages-select drone_planning --event-handlers console_direct+ --return-code-on-test-failure
colcon test --packages-select drone_bringup --ctest-args -R 'test_test_(astar_planner|planned_trajectory)_e2e.py' --event-handlers console_direct+ --return-code-on-test-failure
colcon test --packages-select drone_bringup --ctest-args -R 'test_test_(static_avoidance|multi_goal_static_avoidance)_e2e.py' --event-handlers console_direct+ --return-code-on-test-failure
python3 tools/evaluate_static_avoidance.py --timeout 90
colcon test --event-handlers console_cohesion+ --return-code-on-test-failure
colcon test-result --verbose
...
- 构建：6 个 package 成功；
- 全量测试：`209 tests, 0 errors, 0 failures, 0 skipped`；
- 默认 A*：成功，原始路径 `55` 点、`16.069147 m`，约扩展 `7680` 节点；简化路径 `8` 点、`15.216305 m`；连续轨迹 `43.877966 s`、`velocity_scale=1.00`，点和相邻线段碰撞检查均通过；
- Domain 99 单目标 E2E：`48.194 s` 完成，最大跟踪误差 `0.029918 m`，最小净空 `0.183089 m`，最终位置误差 `0.005209 m`；
- Domain 113 四目标 E2E：索引严格按 `0→1→2→3` 验收，任务时间 `83.957 s`；各段最大跟踪误差 `0.013086/0.015679/0.015579/0.011159 m`，最小净空 `0.209253 m`，最终位置误差 `0.001978 m`，最终速度 `0.000907 m/s`；
- A/B/C 系统评测全部通过；三组中最不利最小净空 `0.178332 m`、最大跟踪误差 `0.031314 m`、最终位置误差 `0.005302 m`；
- 单目标、多目标和三组评测均未发生环境碰撞或独立 Odom 点/线段碰撞，未出现 NaN/Inf，控制器 `saturated=true` 记录为 `0`；任务完成后能够稳定保持最终目标；
- RViz2 节点正常启动，完整执行后无人机稳定在 `(12.1,0.85,1.5)`，默认视角可见工作空间、六障碍和多类路径。

### 遗留问题与下一阶段

- 未通过测试：无；
- Python/C++ E2E 场景夹具仍硬编码六障碍 AABB，后续地图变更必须同步；评测脚本已经读取统一环境配置；
- 四目标完整演示约 `84 s`，比第一版三目标任务更长；默认 A* 扩展节点从约 `4862` 增至约 `7680`，但远低于 `200000` 上限；
- 挑战通道有效宽度仅 `1.2 m`，当前 `0.25 m/s` 多目标速度下已稳定通过，但速度提高后最先需要复核该段的连续轨迹和实际 Odom 线段净空；
- 全高障碍物避免了统一高空绕行，但当前没有真实低矮障碍物上方三维越障；三维效果来自三种目标高度。

下一阶段：在当前复杂场景稳定基线下进行飞行速度优化。

## 远程首目标与偏置返航任务

### 设计目标

为突出远距离自主导航能力，默认多目标任务不再把多个导航点依次布置在前进路线上，而是采用“一次跨地图导航、两段返航”的结构：

| 顺序 | 目标 `(x,y,z,yaw)` | 工程作用 |
|---|---|---|
| P1 | `(12.1,0.85,1.5,0.0)` | 从起飞区直接导航到地图远端，完整穿越六障碍蛇形场景与挑战通道 |
| P2 | `(7.8,3.1,2.5,0.0)` | 返回到地图中部上侧，与直线返航路线形成明显偏置，并增加 `1.0 m` 高度变化 |
| P3 | `(0.8,0.7,1.8,0.0)` | 返回起飞区附近但不与 `(0,0,1.5)` 重合，x/y/z 均有差异 |

几何预检确认三个目标均位于导航地板以上且不在 `0.35 m` 膨胀障碍物内。三段 A* 原始路径分别为 `55/29/38` 点，长度约 `15.9407/7.12313/10.0928 m`，扩展 `6519/1526/2920` 个节点；简化路径分别为 `8/4/7` 点。三段连续轨迹均使用 `velocity_scale=1.0`，纯轨迹时长约 `60.8491/26.5921/36.8292 s`，合计 `124.2704 s`。因此方案可行，但演示时间必然比原四目标前进任务更长。

### 修改与验证

- `multi_goal_mission.yaml` 与测试任务 YAML 改为上述三个目标；
- 多目标 E2E 同步目标、目标计数、路径计数与访问顺序断言；
- 测试内部任务超时从 `105 s` 调整为 `155 s`，CTest 外层超时从 `140 s` 调整为 `180 s`，只用于覆盖更长的真实任务，不修改速度、容差、停稳时间或安全阈值；
- `PlannedTrajectoryBuilder` 和 `CollisionChecker` 测试覆盖新的三段任务与目标安全性；
- 地图、六个障碍物、A* 核心、轨迹算法、控制器、动力学、`nominal_speed=0.25 m/s`、安全半径和规划裕量均未改变。
...
source install/setup.bash
colcon test --packages-select drone_planning --event-handlers console_direct+ --return-code-on-test-failure
colcon test --packages-select drone_bringup --ctest-args -R test_test_multi_goal_static_avoidance_e2e.py --event-handlers console_direct+ --return-code-on-test-failure
```

- 构建：6 个 package 成功；
- `drone_planning`：4 个 CTest 目标全部通过；
- 全量回归：`209 tests, 0 errors, 0 failures, 0 skipped`；
- Domain 113 真实闭环：索引严格按 `0→1→2`，访问计数严格按 `0→1→2→3`；任务时间 `132.091 s`；
- 各段最大跟踪误差 `0.009276/0.010545/0.014433 m`，全程最小净空 `0.128528 m`；
- 三个目标验收误差 `0.004058/0.003670/0.004224 m`，最终位置误差 `0.002811 m`，最终速度 `0.001165 m/s`；
- 实际 Odom 点和相邻线段均无碰撞，环境碰撞状态未出现 true，无 NaN/Inf，控制器 `saturated=true` 日志为 `0`，最终目标持续稳定保持。

### 风险

- 完整演示约 `132 s`，属于该路线长度与保持原速度的直接结果；若演示时间受限，应在后续独立速度优化阶段处理，不能通过删除返航段或降低安全条件伪装缩短；
- 最小净空从前一任务的 `0.209253 m` 降为 `0.128528 m`，仍高于现有 `0.05 m` 额外净空断言且实际点/线段无碰撞；后续提速必须重点复核挑战通道和返航转弯段。

## 实际轨迹长期保持与上侧高点返航

绿色实际飞行轨迹由 `quadrotor_dynamics_node` 在 `/drone/path` 发布。原配置以约 `20 Hz` 采样且只保留 `2000` 点，约 `100 s` 后开始删除最早轨迹，因此超过两分钟的多目标任务会在 RViz2 中逐渐丢失起飞段。本轮不修改动力学实现，只把 `dynamics.yaml` 调整为：

```yaml
path_publish_divider: 20  # 200 Hz / 20 = 10 Hz
path_max_points: 6000     # 约 10 分钟历史
```

该采样密度足以显示连续轨迹，并限制 Path 消息不会无限增长。多目标 E2E 新增 `/drone/path` 订阅，要求长任务结束后历史点数超过 `1000`、首点仍位于起飞原点附近、末点位于最终目标附近。实际闭环结束时保留 `1479` 点，首尾断言通过。

用户建议目标为 P1 `(12.1,5.5,1.5)`、P2 `(7.0,6.0,4.0)`、P3 `(0.8,0.7,2.0)`。三个点本身和三段 A* 均合法，但 P2 距 `0.35 m` 收缩后的工作空间上边界仅 `0.15 m`，且精确方案的 P0→P1、P2→P3 连续轨迹碰撞验证失败。为保持“远端目标—中部上侧高点—偏置返航”意图，采用经过现有算法验证的调整：

| 目标 | 最终坐标 `(x,y,z,yaw)` | 调整原因 |
|---|---|---|
| P1 | `(12.1,1.1,1.5,0.0)` | 保持远端 x 和高度，放入 O5/O6 挑战通道出口的可执行区域 |
| P2 | `(7.0,5.0,4.0,0.0)` | 保持 x 与高点 z，向安全工作空间内部移动 `1.0 m` |
| P3 | `(0.8,0.7,2.0,0.0)` | 用户坐标可直接使用 |

几何预检结果：三段 A* 原始路径 `54/36/39` 点，长度约 `16.0443/10.1537/11.2351 m`，扩展 `6834/4050/3738` 个节点；简化路径 `8/4/7` 点；连续轨迹时长约 `61.0038/35.8210/39.7422 s`，三段均成功。第二段选择 `velocity_scale=0.25` 以满足不变的碰撞和动态约束，其余两段为 `1.0`；`nominal_speed=0.25 m/s` 和所有控制参数均未修改。

最终验证：

- 构建：6 个 package 成功；
- 全量测试：`209 tests, 0 errors, 0 failures, 0 skipped`；
- Domain 113：任务时间 `144.898 s`，索引 `0→1→2`，访问计数 `0→1→2→3`；
- 三段最大跟踪误差 `0.012698/0.018786/0.016702 m`，最小净空 `0.135290 m`；
- 三个目标验收误差 `0.004070/0.003067/0.004265 m`，最终误差 `0.002820 m`，最终速度 `0.001170 m/s`；
- 实际 Path `1479` 点并保留起飞段；实际 Odom 点和相邻线段无碰撞，无 NaN/Inf，控制器饱和日志为 `0`。

风险：完整演示约 `145 s`；Path 缓存是约 10 分钟的有界长期保持，不是无限历史。若需要数小时记录，应使用 rosbag，而不应让 RViz Path 消息无限增长。
