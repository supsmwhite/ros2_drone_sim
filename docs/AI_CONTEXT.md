...
项目采用“纯算法类与 ROS2 节点分离”的结构。当前动力学、高度/yaw 闭环和基础可视化已经运行；x/y 水平位置控制及其与 `HoverController` 的纯算法组合已实现并通过单元测试，也已接入 ROS2 节点。单目标三维闭环尚未通过真实运行验收：持续非零倾角会触发 roll/pitch 自激失稳，因此当前只能称为“接入代码已生成并通过自动化测试”，不能称为“功能已验证”。最终目标仍包括稳定的三维位置控制、轨迹、多目标点、地图、规划和静态避障。

## 2. 当前阶段和下一任务

### 当前阶段

动力学和高度/yaw 闭环已完成；`HorizontalPositionController` 已能在算法层将世界系水平位置/速度误差转换为受限水平加速度和期望姿态。新增的 `PositionController` 按 `HorizontalPositionController → HoverController` 组合控制链，并已接入节点。
...
- 原地起飞、悬停和 yaw 的既有能力仍通过自动化测试，原地 `1.5 m` 悬停也已再次实跑通过。

但是 `(2,1,1.5)` 的真实运行没有通过：三组由强到弱的水平参数只能延迟、不能消除 roll/pitch 自激，随后出现 Mixer 饱和、单电机 0 RPM、姿态和位置发散。三维位置闭环仍未完成。

### 下一任务

先稳定“持续小幅非零倾角”下的 roll/pitch 闭环，再重新验收 `(2,1,1.5)` 单目标。本轮任务禁止修改 roll/pitch 姿态增益，而现有失稳在把水平指令降至 `0.05 m/s²`、最大倾角降至 `0.02 rad` 后仍会发生，因此继续仅调水平参数已不能解决根因。下一步需要用户明确授权后，只针对 roll/pitch 阻尼和电机一阶滞后共同形成的闭环稳定性进行分析、调参和最小范围测试；不得同时进入轨迹、多目标点、地图、规划或避障。
...
- `HorizontalPositionController`：世界系 x/y 位置与速度 PD，向量模长/最大倾角联合限幅，并用期望推力方向和 yaw 航向参考构造 body-to-world 期望姿态；
- `HoverController`：组合高度、姿态和 Mixer；
- `PositionController`：组合水平位置控制和 `HoverController`，传播中间输出及各级 valid/saturated 状态，任一级无效时安全返回零 RPM；
- `position_controller_node` 已实际调用 `PositionController`，但三维闭环真实运行尚未通过。
...
目标仅接受空 frame（按 `map` 处理）或 `map`。当前读取 position.x/y/z 和 orientation 中的 yaw，移除目标 roll/pitch。Odom 姿态用于把完整 `twist.linear` 从 `base_link` 旋转到 `map`，其 x/y/z 分量分别作为水平和垂直速度反馈；`twist.angular` 保持机体系表达直接传入姿态控制器。
...
a_xy_raw = Kp .* e_p + Kd .* e_v
a_limit = min(max_horizontal_acceleration, gravity * tan(max_tilt_angle))
```

当 `|a_xy_raw| > a_limit` 时按向量模长等比例缩放，保持加速度方向不变并设置 `saturated=true`。PD 中间量使用扩展精度和稳定模长计算，极大有限输入在限幅前不会因 double 平方溢出而污染输出。
...
这些值是为诊断失稳而降到很弱的第三组试验参数，不是已验收的运行基线。即使使用这组参数，约 13～15 秒后仍出现 roll/pitch 自激和 Mixer 饱和。此前还实跑过推荐初值 `Kp=1.0,Kd=1.8,a_max=2.0,tilt_max=0.35`，以及中间值 `Kp=0.4,Kd=1.2,a_max=0.4,tilt_max=0.08`，均失败。

## 10. 当前限制和风险

- 水平位置和组合接入代码已通过单元测试，但单目标三维运行未通过；不得把代码存在等同于功能完成；
- 持续非零倾角会引发 roll/pitch 自激，随后 Mixer 持续饱和并出现 0/20000 RPM。现有 `Ixx=Iyy=0.02`、电机时间常数 `0.05 s`、roll/pitch `Kp=4.0,Kd=0.20` 的组合处在简化三阶模型的临界稳定边界附近，采样和其他滞后会进一步降低稳定裕度；此项是基于模型和运行现象的根因判断，尚未通过获准后的增益实验验证；
...
当前未提交工作区另包含 `PositionController`、完整 Odom 速度转换和 ROS2 x/y 接入。它们已通过自动化测试，但真实三维目标运行失败，因此尚未成为稳定功能基线。
...
- 当前工作区汇总为 `95 tests, 0 errors, 0 failures, 0 skipped`；
- 动力学测试覆盖自由落体、地面、起飞/落地、推力/力矩方向和电机响应；
- 控制器测试覆盖 Mixer 11 项、姿态 11 项、高度 13 项、Hover/转换 18 项；
- 水平位置控制器新增 15 项测试，覆盖零误差与 yaw 保留、正负 x/y 符号、速度阻尼、非零 yaw、对角输入、向量模长和倾角限幅、NaN/Inf、非法参数、极大有限输入、四元数单位化及旋转矩阵正交性/行列式；
- `PositionController` 新增 9 项测试，覆盖零水平误差与 Hover 一致性、正 x/正 y 姿态和电机差动方向、速度阻尼、饱和传播、水平/下游无效安全返回、零 x/y 时高度/yaw 不回归和重力参数一致性；
- 完整速度转换新增 4 项测试，覆盖单位姿态、pitch 产生 world z、yaw 旋转 world x/y 和非法四元数；
- 精确 180° 四元数 q/-q 确定性和极大有限 Mixer 输入均已覆盖。

以上结果来自本阶段实际执行的局部和完整构建/测试。还进行了三轮无 RViz 的 ROS2 x/y 闭环运行，但均未通过；没有进行 RViz2 人工验收。
...
本阶段再次实跑确认：先发送 `(0,0,1.5)` 后可稳定在约 `z=1.4999 m`，x/y 接近 0，姿态接近单位四元数，四电机约 `10818.94 RPM`，watchdog 未误触发。

随后发送 `(2,1,1.5)` 的三轮结果均失败：

- 推荐水平参数 `1.0/1.8/2.0/0.35` 很快出现强烈姿态发散和持续饱和；
- 中间参数 `0.4/1.2/0.4/0.08` 前约 4 秒方向正确，约 5～6 秒后自激，15 秒时位置约 `(2.98,-17.05,1.40)`；
- 当前弱参数 `0.1/0.5/0.05/0.02` 前 12 秒缓慢且方向正确，约为 `(1.84,0.92,1.5)`，但约 13～15 秒仍开始自激；观测到位置约 `(2.08,1.05,1.36)`、pitch 角速度约 `3.19 rad/s`、RPM 约 `[0,11208,17289,11208]`，继续运行后发散并反复达到 0/20000 RPM。

因此没有满足最终误差、最终速度、无持续振荡和无持续饱和等验收条件；不存在可报告为“最终稳定值”的位置/速度误差。运行中未观察到 NaN/Inf，但这不构成功能通过。

未把失败的 x/y ROS2 闭环、轨迹跟踪、地图或避障写为已验证。
...
7. 节点已开始使用目标 x/y；在真实运行通过前不得在 README 中宣称三维位置控制已验证；
...
当前仅第一项中的原地悬停回归通过；非零 x/y 会先产生正确方向的倾斜和运动，但随后姿态持续发散，故其余系统级条件未通过。下一轮应在获得修改 roll/pitch 阻尼的明确授权后，先增加非零倾角闭环回归，再重复本节验收。
