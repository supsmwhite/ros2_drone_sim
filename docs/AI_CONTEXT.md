...
- Domain 113 有序多目标真实闭环依次执行远端 P1 `(12.1,0.85,1.5)`、中部上侧返航点 P2 `(7.8,3.1,2.5)`、起飞区偏置返航点 P3 `(0.8,0.7,1.8)`，任务于 `132.091 s` 完成；三段最大跟踪误差 `0.009276/0.010545/0.014433 m`，最小净空 `0.128528 m`，最终误差 `0.002811 m`、速度 `0.001165 m/s`；共采集 Odom/setpoint/RPM/碰撞状态 `27014/6753/13507/2702` 个样本，完成后继续收到 `600/150/300` 个 Odom/setpoint/RPM 样本；环境碰撞未出现 true，独立 Odom 点和连续线段检查均安全，无 NaN/Inf，控制器日志 `saturated=true` 为 `0` 次，节点和最终 setpoint 持续存活；
