...
- `multi_goal_static_avoidance_sim.launch.py` 只启动动力学、轨迹模式控制器、robot_state_publisher、可选 RViz2、静态环境和多目标任务节点；`mission_config` 默认指向四目标 `multi_goal_mission.yaml`；
...
- `PlannedTrajectoryBuilder` 测试除原有默认场景、安全与动态约束、精确首尾、确定性、候选下降与非法参数外，还覆盖 `0.50 m` 导航地板、首次实际起飞 Odom 和默认四目标全部有序段；
...
- `test_multi_goal_static_avoidance_e2e.py` 使用独立 `ROS_DOMAIN_ID=113`，真实执行 P1/P2/P3/P4，检查首次实际 Odom 起飞、索引与已访问计数严格顺序、每段规划起点来自稳定实际 Odom、三类 Path 数量和导航地板、逐目标停止验收、最终保持、发布者/节点独占、消息活性，以及独立 Odom 点/闭 slab 连线碰撞、跟踪误差和净空；
...
- Domain 113 有序多目标真实闭环依次执行 P1 `(3.6,3.1,1.5)`、P2 `(7.85,3.1,2.5)`、P3 `(9.85,0.6,3.25)`、P4 `(12.1,0.85,1.5)`，任务于 `83.957 s` 完成；四段最大跟踪误差 `0.013086/0.015679/0.015579/0.011159 m`，最小净空 `0.209253 m`，最终误差 `0.001978 m`、速度 `0.000907 m/s`；共采集 Odom/setpoint/RPM/碰撞状态 `17386/4347/8694/1739` 个样本，完成后继续收到 `600/151/301` 个 Odom/setpoint/RPM 样本；环境碰撞未出现 true，独立 Odom 点和连续线段检查均安全，无 NaN/Inf，控制器日志 `saturated=true` 为 `0` 次，节点和最终 setpoint 持续存活；
- 当前四目标任务已由 Domain 113 闭环回归确认严格按序执行；RViz2 保留多目标路径、实际轨迹和环境显示，当前六障碍地图的定量验收以自动测试和评测数据为准；
