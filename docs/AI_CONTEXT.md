...
专用 `disturbance_demo.rviz` 固定 frame 为 `map`，预置 RobotModel、TF、Odom、实际 Path、目标 Pose 和扰动 MarkerArray。稳定 namespace/ID 的红色箭头表示 **Equivalent External Force**（集中等效外力，不是真实风速或完整风场）；蓝色箭头表示水平位置积分产生的世界系加速度补偿（不是实际作用力）；细线连接实际位置与悬停目标。零力或近零积分使用 DELETE，不对零向量归一化。专用 Launch 复用正式 `dynamics.yaml`/`controller.yaml`，只对动力学覆盖 `enable_external_wrench=true`；旧 `disturbance_hover_sim` 同样是专用扰动实验入口，`basic_sim`、多目标和交互导航等正常 Launch 继续使用默认 false，障碍地图中没有施加扰动。

TEXT_VIEW_FACING 已改为演示优先的精简状态：等待/起飞 `TAKEOFF / SETTLING` 1 行，倒计时 `GUST IN ...` 1 行，扰动阶段正常 3 行（剩余时间、F/e_xy、I_xy），恢复阶段 2 行，完成阶段 1 行。正常不显示全零 Saturation；仅 H/Z/ATT/MIXER 任一饱和时追加一行 `WARNING`，多个饱和合并在同一行，因此绝对最多 4 行。字号从 `0.20` 调整到 `0.16`；外力/积分箭头的颜色和长度比例、目标点、误差线、阶段机、外力时序及全部安全检查未改。

本阶段冻结现有水平积分算法、卸载逻辑和全部正式控制参数；高度环与姿态环继续为 PD，没有高度积分或姿态积分，也没有重新进行参数扫描。外力仍只是质心处集中等效力，不能描述为风场。

上一阶段相关回归执行 `colcon test --packages-select drone_bringup drone_controller`：controller 6/6、bringup 16/16 测试入口通过，随后 `colcon test-result --verbose` 汇总为 `288 tests, 0 errors, 0 failures, 0 skipped`。另执行一次缩短时序的专用 RViz 烟雾演示，RViz/OpenGL 正常启动，阶段走完 ACTIVE、RECOVERY、COMPLETE，无控制饱和；该烟雾演示不替代 README A～J 的用户人工验收。

README 人工验收现按 A～J 排列：编译、基础悬停、单目标 `(2,1,1.5)`、复用 `mission_sim.launch.py`/`mission.yaml` 的无障碍多目标顺序飞行、短时扰动、持续扰动、静态多目标避障、RViz 交互导航、预检失败安全、最终全量测试。每项区分测试目的、主命令、必要操作、RViz 重点、通过标准和可选诊断；日常演示只运行主命令，Topic/参数检查移动到末尾可选深度诊断附录。没有新增任务配置、Launch 或任务算法。

本阶段定向回归先用 `ctest -N` 确认真实测试名，再执行 `colcon test --packages-select drone_bringup --ctest-args -R "disturbance_demo|single_goal|waypoint"`，结果汇总为 `275 tests, 0 errors, 0 failures, 0 skipped`。默认 `ros2 launch drone_bringup disturbance_visual_demo.launch.py profile:=short_gust` RViz 烟雾演示完整走过倒计时、ACTIVE、RECOVERY、COMPLETE，OpenGL/RViz 正常，Marker 读回确认字号 `0.16`、完成态 1 行、撤力后红箭头 DELETE、蓝色积分箭头及目标/误差线保留；全量测试留待用户完成 A～I 后执行。
