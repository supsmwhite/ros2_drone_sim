...
- `multi_goal_static_avoidance_sim.launch.py` 只启动动力学、轨迹模式控制器、robot_state_publisher、可选 RViz2、静态环境和多目标任务节点；`mission_config` 默认指向三目标 `multi_goal_mission.yaml`；
...
- `PlannedTrajectoryBuilder` 测试除原有默认场景、安全与动态约束、精确首尾、确定性、候选下降与非法参数外，还覆盖 `0.50 m` 导航地板、首次实际起飞 Odom 和默认三目标全部有序段；
...
- `test_multi_goal_static_avoidance_e2e.py` 使用独立 `ROS_DOMAIN_ID=113`，真实执行 P1/P2/P3，检查首次实际 Odom 起飞、索引与已访问计数严格顺序、每段规划起点来自稳定实际 Odom、三类 Path 数量和导航地板、逐目标停止验收、最终保持、发布者/节点独占、消息活性，以及独立 Odom 点/闭 slab 连线碰撞、跟踪误差和净空；
...
- 静态环境集成运行已确认 `workspace/obstacles/inflated_obstacles` Marker 分类、碰撞 true/false 转换，以及非法或超时 Odom 不发布状态；RViz Orbit 焦点为 `(6,2,1.5)`、距离 `16 m`，五个原始障碍物、膨胀区和完整任务路径可处于同一默认视野；
- 默认 A* ROS2 集成使用 `0.35 m` 有效规划半径，输出 `51` 个路径点、总长 `14.624585 m`、扩展 `4862` 个节点、最大高度 `1.6 m`，规划耗时在本机约 `0.46～0.51 s`；
- 默认规划轨迹集成将 `51` 个原始点简化为 `7` 个点，折线长度降为 `14.022856 m`；选择 `velocity_scale=1.00`，总时长 `40.065302 s`，最大速度 `0.548638 m/s`、最大加速度 `0.178899 m/s²`、验证采样 `2005` 个，独立几何复核无轨迹碰撞；参考 Path 为 `803` 个点；
...
- Domain 99 静态避障完整回归中，起点准备后按段索引 `0→1→2→3→4→5` 执行，任务于 `44.110 s` 完成；最大跟踪误差 `0.029911 m`，实际 Odom 对基础膨胀障碍物的最小净空 `0.218112 m`，控制器日志 `saturated=true` 为 `0` 次；最终误差 `0.004557 m`、速度 `0.001606 m/s`，全程无碰撞或 NaN/Inf；
...
|---|---|---:|---:|---:|---:|---:|---:|
| A 默认蛇形 | `(12,2.7,1.5)` | `51 / 14.624585` | `7 / 14.022856` | `40.065302 / 1.00` | `0.548638 / 0.178899` | `0.029521 / 0.218269` | `0.004415 / 0.001542` |
| B 水平终点 | `(12,3.2,1.5)` | `51 / 14.831692` | `7 / 14.108087` | `40.308820 / 1.00` | `0.548638 / 0.178899` | `0.030909 / 0.218109` | `0.004215 / 0.001514` |
| C 三维高度 | `(12,2.7,4.0)` | `51 / 15.419178` | `7 / 14.457766` | `41.307904 / 1.00` | `0.551851 / 0.167280` | `0.029521 / 0.218237` | `0.004202 / 0.001440` |
...
- Domain 113 有序多目标真实闭环依次执行 P1 `(4.2,2.5,1.5)`、P2 `(7.4,0,1.5)`、P3 `(12,2.7,1.5)`，任务于 `67.690 s` 完成；三段最大跟踪误差 `0.009384/0.010359/0.009494 m`，最小净空 `0.240352 m`，最终误差 `0.001466 m`、速度 `0.000684 m/s`，无碰撞、非有限值或控制饱和；
- 同一回归的三段最大同期跟踪误差为 `0.016959/0.007322/0.008700 m`，实际最小采样净空 `0.147244 m`，任务于 `71.719 s` 完成，最终误差 `0.001617 m`、速度 `0.000730 m/s`；共采集 Odom/setpoint/RPM/碰撞状态 `14939/3735/7470/1494` 个样本，完成后继续收到 `600/150/300` 个 Odom/setpoint/RPM 样本。环境碰撞未出现 true，独立 Odom 点和连续线段检查均安全，无 NaN/Inf，控制器日志 `saturated=true` 为 `0` 次，节点和最终 setpoint 持续存活；
- 当前三目标任务已由 Domain 113 闭环回归确认严格按序执行；RViz2 保留多目标路径、实际轨迹和环境显示，当前五障碍地图的定量验收以自动测试和评测数据为准；
