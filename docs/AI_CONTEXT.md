...
- 最终全量回归任务时间：`142.175 s`；旧速度本轮复测 `183.894 s`，缩短 `41.719 s`（`22.69%`）；
...
- 三段最大跟踪误差：`0.021435/0.028343/0.019884 m`；
- 三段最小基础净空：`0.089752/0.138795/0.201006 m`，全局最小 `0.089752 m`；
- 最终误差：`0.004157 m`；最终速度：`0.001689 m/s`；
- 实际轨迹历史：1452 点，任务结束时仍包含起飞初始点；
- 无碰撞、非有限值或控制器饱和。
...
- 远端 P1 需要 3 轮局部路径细化，最终多目标实跑最小基础净空 `0.089752 m`，已接近本轮 `0.085 m` 选择下限；不建议继续统一提高名义速度。
