...
def main():
    ...
    summary={"schema_version":3,"experiment":experiment,"status":meta["status"],"repository_commit":meta["repository_commit"],"sample_count":len(rows_all),"mission_sample_count":len(rows),"navigation_phase_start_time_s":navigation_start,"navigation_phase_start_source":navigation_source,"metrics":metrics,"yaw_analysis":{"status":yaw_stats["status"],"reference_field":yaw_key,"error_definition":"wrap_to_pi(reference_yaw - actual_yaw)","completion_gate_yaw_tolerance_rad":yaw_gate,"completion_gate_source":f"{gate_file}:{gate_key}","acceptance_threshold_applied":False},"assignment_thresholds":{"hover_final_position_error_max_m":.3},"project_thresholds":{"final_position_error_max_m":.1,"minimum_safety_clearance_strictly_greater_than_m":0.,"non_finite_value_count":0,"attitude_divergence_detected":False,"saturated_at_end":False},"legacy_checks":legacy_checks,"checks":checks,"overall_pass":overall_pass,"failure_reasons":failure_reasons}
