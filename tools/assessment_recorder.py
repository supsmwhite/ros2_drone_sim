...
def target_from_value(value):
    if isinstance(value, dict):
        position = value.get("position")
        if position is None:
            position = [value[key] for key in ("x", "y", "z")]
        return normalized_target(position, value.get("yaw_rad", value.get("yaw")))
    if isinstance(value, (list, tuple)) and len(value) >= 3:
        return normalized_target(value[:3], value[3] if len(value) >= 4 else None)
    raise ValueError("each target must contain x/y/z and an optional yaw")


def configured_targets(path, command_line_targets):
    if command_line_targets:
        return [target_from_value(value) for value in command_line_targets]
    if not path:
        return []
    data = yaml.safe_load(Path(path).read_text())
    if isinstance(data, dict):
        data = data.get("targets", data.get("goals", data.get("target", data)))
    values = data if isinstance(data, list) and data and isinstance(data[0], (list, tuple, dict)) else [data]
    return [target_from_value(value) for value in values]


def default_service_name(experiment):
    if experiment in ("hover", "single_goal", "multi_goal"):
        return "/drone/mission/execute"
    if experiment in ("navigation", "failure_case"):
        return "/drone/interactive_goals/execute"
    return None


class Recorder:
    def __init__(self,node,args):
        self.node,self.args=node,args
        self.protocol_targets=configured_targets(args.target_config,args.expected_goal)
        self.boxes,self.safety_radius=environment(args.environment_config)
        self.output=prepare_output_directory(args.output,args.overwrite_existing,args.run_status)
        ...
        self.ignored_goal_count=0; self.pending_rows=[]; self.diagnostic_samples=[]
    ...
    def set_current_target(self,target):
        self.current_goal=list(target["position"]); self.current_goal_yaw=target.get("yaw_rad")
        self.final_goal=list(target["position"]); self.final_goal_yaw=target.get("yaw_rad")

    def populate_goal_fields(self,row,actual,yaw):
        if self.current_goal is None:return None,None
        error=[self.current_goal[i]-actual[i] for i in range(3)]
        magnitude=math.sqrt(sum(value*value for value in error))
        row.update({"goal_x":self.current_goal[0],"goal_y":self.current_goal[1],"goal_z":self.current_goal[2],
          "goal_error_x":error[0],"goal_error_y":error[1],"goal_error_z":error[2],"goal_position_error":magnitude})
        if self.current_goal_yaw is not None:
            row["goal_yaw"]=self.current_goal_yaw
            row["yaw_error"]=math.remainder(self.current_goal_yaw-yaw,2*math.pi)
        return error,magnitude

    def flush_pending_rows(self):
        for now,row,actual,yaw in self.pending_rows:
            if self.mission_start is not None and now+1e-9>=self.mission_start:
                row["mission_time_s"]=max(0.,now-self.mission_start)
                self.populate_goal_fields(row,actual,yaw)
    ...
    def on_diag(self,m):
        self.tick("/drone/controller/diagnostics"); self.diag=m
        now=self.node.get_clock().now().nanoseconds*1e-9
        flags={"horizontal_saturated":bool(m.horizontal_saturated),"altitude_saturated":bool(m.altitude_saturated),
          "attitude_saturated":bool(m.attitude_saturated),"mixer_saturated":bool(m.mixer_saturated)}
    ...
    def on_goal(self,m):
        ...
        self.set_current_target(observed)
        if not self.observed_targets:self.observed_targets=[observed]
        if repeated:return
    ...
    def on_reference(self,m):
        ...
        if self.args.experiment=="disturbance":
            ...
            self.start_mission("trajectory_started",self.last_ros)
    ...
    def on_mission_index(self,m):
        self.tick("/drone/mission/current_waypoint_index")
        if self.args.experiment not in ("hover","single_goal","multi_goal"):return
        self.mission_index=int(m.data); self.changed("waypoint_index_changed",self.mission_index)
        targets=self.observed_targets or self.protocol_targets
        if targets and self.mission_index<len(targets): self.set_current_target(targets[self.mission_index])
        self.activate_goal(self.mission_index,"mission_waypoint_index")
    def on_mission_complete(self,m):
        self.tick("/drone/mission/complete")
        if self.args.experiment not in ("hover","single_goal","multi_goal"):return
        self.mission_complete=bool(m.data); self.changed("mission_complete_changed",self.mission_complete)
    def on_navigation_goal(self,m):
        self.tick("/drone/multi_goal/current_goal_pose")
        if self.args.experiment not in ("navigation","failure_case"):return
        target=normalized_target([m.pose.position.x,m.pose.position.y,m.pose.position.z],euler(m.pose.orientation)[2]); self.set_current_target(target)
        index=0 if self.navigation_index is None else self.navigation_index
        while len(self.observed_targets)<=index:self.observed_targets.append(None)
        self.observed_targets[index]=target
    def on_nav_index(self,m):
        self.tick("/drone/multi_goal/current_goal_index")
        if self.args.experiment not in ("navigation","failure_case"):return
        self.navigation_index=int(m.data); self.changed("navigation_goal_index_changed",self.navigation_index)
        self.activate_goal(self.navigation_index,"navigation_goal_index")
    def on_visited(self,m):
        self.tick("/drone/multi_goal/visited_goals")
        if self.args.experiment in ("navigation","failure_case"):self.visited=int(m.data); self.changed("navigation_visited_goals_changed",self.visited)
    def on_nav_complete(self,m):
        self.tick("/drone/multi_goal/complete")
        if self.args.experiment in ("navigation","failure_case"):self.nav_complete=bool(m.data); self.changed("navigation_complete_changed",self.nav_complete)
    def on_nav_success(self,m):
        self.tick("/drone/multi_goal/success")
        if self.args.experiment in ("navigation","failure_case"):self.nav_success=bool(m.data); self.changed("navigation_success_changed",self.nav_success)
    def on_ready(self,m):
        self.tick("/drone/interactive_goals/ready")
        if self.args.experiment in ("navigation","failure_case"):self.ready=bool(m.data); self.changed("interactive_ready_changed",self.ready)
    def on_count(self,m):
        self.tick("/drone/interactive_goals/count")
        if self.args.experiment in ("navigation","failure_case"):self.goal_count=int(m.data); self.changed("interactive_goal_count_changed",self.goal_count)
    def on_active(self,m):
        self.tick("/drone/interactive_mission/active")
        if self.args.experiment not in ("navigation","failure_case"):return
        self.active=bool(m.data)
        if self.active:
            if self.current_goal is None and self.protocol_targets:self.set_current_target(self.protocol_targets[0])
            self.start_mission("interactive_mission_active",self.last_ros)
    ...
    def on_odom(self,m):
        ...
        actual=[p.x,p.y,p.z]; body_velocity=[v.x,v.y,v.z]
        q=m.pose.pose.orientation; map_velocity=rotate_body_velocity_to_map([q.x,q.y,q.z,q.w],body_velocity)
        speed=math.sqrt(v.x*v.x+v.y*v.y+v.z*v.z); self.maximum_altitude=max(self.maximum_altitude,p.z); self.maximum_speed=max(self.maximum_speed,speed)
        def err(target):
        ...
        row={k:"" for k in FIELDS}; row.update({"recording_time_s":now-self.record_start,"mission_time_s":"" if self.mission_start is None else now-self.mission_start,
          "actual_x":p.x,"actual_y":p.y,"actual_z":p.z,
          "body_velocity_x":body_velocity[0],"body_velocity_y":body_velocity[1],"body_velocity_z":body_velocity[2],
          "map_velocity_x":map_velocity[0],"map_velocity_y":map_velocity[1],"map_velocity_z":map_velocity[2],"speed":speed,
    ...
    def finish(self):
        ...
        self.event("recording_stopped",{"reason":self.controller.stop_reason})
        if self.pending_rows:self.flush_pending_rows()
        ...
        metadata={"schema_version":3,"protocol_version":self.args.protocol_version,
          ...
          "goals":[target["position"] for target in targets],"goal_order":self.goal_order,
          "ignored_nonmatching_goal_count":self.ignored_goal_count,
          "signal_semantics":{"odom_twist_frame":"base_link","body_velocity_fields":["body_velocity_x","body_velocity_y","body_velocity_z"],
            "map_velocity_fields":["map_velocity_x","map_velocity_y","map_velocity_z"],
            "motor_rpm_topic":"/drone/motor_rpm_cmd","motor_rpm_quantity":"commanded_motor_rpm",
            "saturation_count_source":"diagnostics.csv callbacks"},
          "safety_radius_m":self.safety_radius,"failure_reason":self.controller.failure_reason or self.failure_reason,
        ...
        with (self.output/"diagnostics.csv").open("w",newline="") as f:
            fields=("recording_time_s","mission_time_s","horizontal_saturated","altitude_saturated","attitude_saturated","mixer_saturated","any_saturated")
            w=csv.DictWriter(f,fieldnames=fields); w.writeheader()
            for sample in self.diagnostic_samples:
                timestamp=sample["time_s"]
                w.writerow({"recording_time_s":"" if self.record_start is None or timestamp<self.record_start else timestamp-self.record_start,
                  "mission_time_s":"" if mission_relative_time(timestamp,self.mission_start) is None else mission_relative_time(timestamp,self.mission_start),
                  **{key:int(value) for key,value in sample.items() if key!="time_s"}})
...
def main():
    a=arguments(); rclpy.init(); n=rclpy.create_node("assessment_recorder")
    try:rec=Recorder(n,a)
    except (FileExistsError,ValueError) as error:
        n.get_logger().error(str(error)); n.destroy_node(); rclpy.shutdown(); return 2
