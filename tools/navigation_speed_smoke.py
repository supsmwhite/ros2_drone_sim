...
def analyze(run_dir, scenario, candidate, parameters, environment_path, commit, branch):
    ...
    for row in navigation_rows:
        ...
        if all(value is not None for value in velocity):
            reference_speeds.append(math.sqrt(sum(value * value for value in velocity)))
        if all(value is not None for value in acceleration):
            reference_accelerations.append(
                math.sqrt(sum(value * value for value in acceleration)))
        if all(value is not None for value in commanded):
            commanded_accelerations.append(math.hypot(*commanded))
    ...
    collisions = collision_count(actual_points, safety_boxes)
    collisions += int(any(truth(row, "collision_state") is True for row in rows))
    ...
    if collisions:
        failure_reasons.append(f"collision_count={collisions}")
    if non_finite_count:
